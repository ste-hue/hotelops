...
|---|---|---|---|---|---|---|---|---|---|---|
| HotelOps Hub (Streamlit app-store: cashflow, bilancini, accodamenti, mutui-embed, revenue, fb, spiaggia, reviews, banche-bind) | `hotelops-hub-942784312622.europe-west1.run.app` (+legacy alias `…lvk3phpq4q-ew.a.run.app`) | none (direct run.app) | IAP + Cloud Run IAM (IAP agent only) + `roles.py` grants | `hotelops-hub` | hotelops-suite | europe-west1 | `hotelops` repo, `Dockerfile` → `verticals/hub/app.py` | manual `gcloud run deploy --source .` | active | high |
