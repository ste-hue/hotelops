...
Reads raw mastrino Excel files from datahub ingresso/banca/mastrino_grezzo/,
transforms them to 5D fact rows, appends to fatti/f_ledger_movimenti.csv.

Usage:
    python -m pipelines.banca.ingest_mastrino --datahub /path/to/datahub --all
    python -m pipelines.banca.ingest_mastrino --datahub /path/to/datahub --file FILENAME
    python -m pipelines.banca.ingest_mastrino --datahub /path/to/datahub --dry-run --all
"""
...
def main():
    ...
    parser.add_argument("--file", "-f", help="Process specific file in ingresso/banca/mastrino_grezzo/")
    ...
    ingresso = datahub / "ingresso" / "banca" / "mastrino_grezzo"
    files = []
    if args.file:
        ...
        files = sorted(ingresso.glob("*.xls*"))
    else:
        parser.print_help()
        sys.exit(1)
