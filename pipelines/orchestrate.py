...
Usage:
    # Full run (sync + ingest all)
    python -m pipelines.orchestrate

    # Dry run — parse everything, write CSVs, no BigQuery
    python -m pipelines.orchestrate --dry-run

    # Skip Drive sync (use local staging only)
    python -m pipelines.orchestrate --no-sync

    # Only run specific pipeline groups
    python -m pipelines.orchestrate --only banca
    python -m pipelines.orchestrate --only amministrativa
    python -m pipelines.orchestrate --only dimensioni

    # Verbose output
    python -m pipelines.orchestrate --dry-run --verbose

Architecture:
    ingresso/ (Drive)
    ├── TESORERIA_ingresso/         → ingest.py (banche)
    │   └── ESOLVER/{INTUR,ORTI}/  → ingest_movimenti_contabili.py
    ├── amministrativa_ingresso/
    │   └── accodamenti/            → ingest_accodamenti.py
    ├── economato_ingresso/         → ingest_consumi_economato.py
    ├── coperti_ingresso/           → ingest_coperti.py
    └── (manual files)
        ├── bilancino_*.xls         → ingest_bilancino.py
        ├── Gasparotto_*.xlsx       → ingest_gasparotto.py
        └── PianoFinanziario_*.xlsx → ingest_piano_finanziario_xlsx.py
"""
...
STAGING = {
    "tesoreria":    STAGING_BASE / "tesoreria_staging",
...
def _banca_args(ctx: dict) -> list[str]:
    args = ["--datahub", str(ctx["datahub"]), "--source", str(STAGING["tesoreria"])]
    if ctx["dry_run"]:
        args.append("--dry-run")
    if ctx["verbose"]:
        args.append("--verbose")
...
def _economato_args(ctx: dict) -> list[str]:
    source = ctx["datahub"] / "ingresso" / "economato_ingresso"
    args = [
        "--source", str(source),
        "--datahub", str(ctx["datahub"]),
    ]
...
def _coperti_args(ctx: dict) -> list[str]:
    source = ctx["datahub"] / "ingresso" / "coperti_ingresso"
    args = [
        "--source", str(source),
        "--datahub", str(ctx["datahub"]),
    ]
...
def _bilancino_discover(ctx: dict) -> list[list[str]]:
    """Discover bilancino files and return arg lists for each."""
    ingresso = ctx["datahub"] / "ingresso" / "bilancino_ingresso"
    if not ingresso.exists():
        return []
    runs = []
    for f in sorted(ingresso.glob("*.xls*")):
        if f.name.startswith("~$"):
            continue
        # Infer societa and mese from filename
        # Expected: bilancino_ORTI_2026-01.xls or similar
        name_upper = f.name.upper()
        societa = "ORTI" if "ORTI" in name_upper else "INTUR" if "INTUR" in name_upper else None
        # Try to find YYYY-MM in filename
        mese = None
        import re
        m = re.search(r"(\d{4}-\d{2})", f.name)
        if m:
            mese = m.group(1)
        if not societa or not mese:
            continue  # Can't infer — skip, log warning
...
def _gasparotto_discover(ctx: dict) -> list[list[str]]:
    ...
    ingresso = ctx["datahub"] / "ingresso" / "budget_ingresso"
    if not ingresso.exists():
        return []
    runs = []
...
def _piano_fin_discover(ctx: dict) -> list[list[str]]:
    ...
    ingresso = ctx["datahub"] / "ingresso" / "piano_finanziario_ingresso"
    if not ingresso.exists():
        return []
    args = ["--dir", str(ingresso), "--latest-only"]
    if ctx["dry_run"]:
        args += ["--dry-run", "--output-dir", str(ctx["output_dir"])]
    return [args] if list(ingresso.glob("*.xlsx")) else []


def _dim_categorie_args(ctx: dict) -> list[str]:
    # Look for the canonical file
    candidates = [
        ctx["datahub"] / "ingresso" / "dimensioni_ingresso" / "Costi Ricavi 2025-2026 Budget.xlsx",
        ctx["datahub"] / "ingresso" / "budget_ingresso" / "Costi Ricavi 2025-2026 Budget.xlsx",
    ]
    for c in candidates:
        if c.exists():
            args = ["--file", str(c)]
...
def _sync_banca(ctx: dict) -> None:
    ...
    subprocess.run(
        [sys.executable, "-m", "pipelines.banca.fetch_drive",
         "--staging", str(STAGING["tesoreria"])],
...
PIPELINES = [
    ...
    Pipeline(
        name="accodamenti",
        group="banca",
        module="pipelines.banca.ingest_accodamenti",
    ...
    Pipeline(
        ...
        module="pipelines.amministrativa.ingest_movimenti_contabili",
    ...
    Pipeline(
        ...
        group="amministrativa",
        module="pipelines.amministrativa.ingest_consumi_economato",
    ...
    Pipeline(
        name="coperti",
        group="amministrativa",
        module="pipelines.amministrativa.ingest_coperti",
    ...
    Pipeline(
        name="categorie",
        group="dimensioni",
        module="pipelines.amministrativa.ingest_categorie",
    ...
    Pipeline(
        name="piano_conti",
        group="dimensioni",
        module="pipelines.amministrativa.ingest_piano_conti_nuovo",
...
MULTI_PIPELINES = [
    ("bilancino", "amministrativa",
     "pipelines.amministrativa.ingest_bilancino",
     _bilancino_discover,
     "Trial balance → f_bilancino"),
    ("gasparotto", "amministrativa",
     "pipelines.amministrativa.ingest_gasparotto",
     _gasparotto_discover,
     "Gasparotto budget → f_budget_mensile"),
    ("piano_finanziario", "amministrativa",
     "pipelines.amministrativa.ingest_piano_finanziario_xlsx",
     _piano_fin_discover,
