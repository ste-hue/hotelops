...
- 2026-04-11: reviews **Layer 2 single-write fix** (`0e5824a`) — scoperto a runtime via scrape reale Trip.com: ogni run OK lasciava la riga stuck RUNNING perché l'UPDATE su `__exit__` colpiva sempre lo streaming buffer di BQ. Refactor a event-sourcing: niente RUNNING placeholder, una sola INSERT su `__exit__` con lo stato finale (OK/FAIL). `check_crashed_runs` + status `RUNNING` rimossi (dead code). Crash processi non lasciano riga ma `check_pipeline_staleness` (36h) copre già. 15/15 test verdi, ruff clean. Riga orfana RUNNING in `f_pipeline_runs` ancora nel buffer, si svuota da sola in ~90min.
- 2026-04-11: reviews **scrape reale Trip.com** — 37 review caricate end-to-end (HOTEL 15/avg 8.75, RESIDENCE 15/8.33, CVM 7/4.57). `normalize_trip` valida: rating 1-10 pass-through OK, hash cross-hotel isolato, review anonime. CVM 4.57 sotto threshold alert — da investigare.
- 2026-04-11: infra **`f_pipeline_runs` creata** — PARTITION BY DATE(started_at), CLUSTER BY pipeline_name, status.
- 2026-04-11: reviews **Layer 2 f_pipeline_runs observability** (merge commit + `b9f46a3` + `72b5e20`) — `PipelineRun` context manager in `core/pipeline_run.py`, `PipelineRunRow` schema, `F_PIPELINE_RUNS` constant. `_cmd_scrape` wrappato con cattura rows_found/rows_new/alerts_sent/usage_total_usd/watermark meta. `scrape_platform`/`scrape_all` ora ritornano `(items, total_cost_usd)` tuple. Health check `check_watermark_staleness` (21d) + `check_pipeline_staleness` (36h) wirati in `cmd_health`. 15 test. **Design flaw scoperto a runtime** — vedi fix `0e5824a` sopra.
