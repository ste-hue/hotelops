...
> **Triage thread 2026-06-12** (skill hotelops-threads, prima run): **PURSUE top-1** = cash-pf-engine Metà A (spec+plan pronti su `.worktrees/cash-pf-engine`, spina cash_control, P0/P1 chiusi); **top-2** = dashboard Streamlit F&B (sessione viva su `worktree-fb-looker`, spec approvata oggi). **HANDOFF** (~20 min manuali, sbloccano 3 fronti): email a Lara (revman), `createAuditForm()`+FORM_URL+hosting (audit direttore), Rosa stipendi maggio ORTI + re-export bilancini 2025 con CE (BVA), export homebanking per banche stale. **Da decidere**: `feat/fb-looker-close` si sovrappone al fronte FB della sessione Streamlit → assorbire/chiudere, non lavorarci in parallelo. Worktree: `app-store` (hub, brainstorm vivo 12/06) e `fb-looker` ATTIVI — niente prune; migrazione dal path legacy `.claude/worktrees/` a fine thread.
