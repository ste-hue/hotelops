...
- **GCS come Raw layer immutabile** (deciso 2026-05-02, **IMPLEMENTATO per pilot 2026-05-05** PR #8): bucket `hotelops-raw` con Object Versioning + Autoclass terminal=ARCHIVE è il Raw layer per **TUTTO il contenuto Drive datahub**. Pilot `MPS_BANCA_ORTI_APPEND` live in produzione. Drive datahub resta come spazio umano editabile. Resta aperto il rollout: bulk flip altre 12 sources `backend: gcs` quando il loop le richiama (discrimination-need rule).
