...
| Export Power BI per-struttura senza BU nel contenuto: il footer la dichiara per NEGAZIONE ("CodiceHotel is not X or Y") | BU = complemento del set escluso (content-only); il parser DEVE esplodere se il complemento non è univoco — mai default silenzioso (vedi `ingest_consprev_mensile.detect_bu_from_footer`) |
| Source portafoglio/forecast (OTB, Cons+Prev): la foto nuova NON sostituisce la vecchia — il pace È il confronto tra foto | `snapshot_date` (da intake_at, mai nel contenuto) nella natural key; varianti della stessa foto (es. dim_tipologia) in chiave e MAI sommate. Decisione `2026-07-11_Promotion_Prenotazioni_Snapshot_Pace` |
