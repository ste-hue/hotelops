...
Legge tutti gli xlsx da datahub/economato/ (struttura a cartelle
...
economato/ e riesegui — solo le righe nuove vengono aggiunte.
...
# Source = datahub/economato/ (struttura a cartelle per reparto)
SOURCE_DEFAULT = DATAHUB_DEFAULT / "economato"
...
def resolve_reparto(folder_name: str, mapping: dict[str, dict]) -> dict:
    ...
    evento_nome = re.sub(
        r"[\s_]+consumi[\s_]+\d{4}$", "", folder_name, flags=re.IGNORECASE
    ).strip()
...
def parse_xlsx(path: Path, anno: int, dim: dict, logger: logging.Logger) -> list[dict]:
    ...
    for i, row in enumerate(rows_raw):
        if (
            row
            and row[0]
            and str(row[0]).strip().lower() in ("codice", "cod.", "reparto")
        ):
    ...
    fmt_2024 = header[0].lower() == "reparto" and not any(
        h.lower() == "data" for h in header
    )
    ...
    if dim.get("is_evento") and evento_nome.lower() in (
        "eventi",
        "eventi 2024",
        "eventi 2025",
        "eventi 2026",
    ...
    for row in rows_raw[data_start:]:
        ...
        codice = (
            str(row[col_codice]).strip() if col_codice >= 0 and row[col_codice] else ""
        )
        ...
        importo = (
            float(row[col_importo])
            if col_importo >= 0 and isinstance(row[col_importo], (int, float))
            else 0.0
        )
        ...
        if fmt_2024:
            ...
            subcat = (
                str(row[col_subcat]).strip()
                if col_subcat >= 0 and row[col_subcat]
                else ""
            )
            reparto_r = (
                str(row[ci("reparto")]).strip()
                if ci("reparto") >= 0 and row[ci("reparto")]
                else ""
            )
...
def collect_all(
    source: Path, anno: int, mapping: dict[str, dict], logger: logging.Logger
) -> list[dict]:
