...
- `f_produzione_pms` — produzione giornaliera HotelCube per classe (**Daily Production Report granulare**, Imponibile, source `POWERBI_PRODUZIONE_ORTI_SNAPSHOT`). Classi spiaggia: `04BEALL` (ombrelloni alloggiati hotel), `10BEBAR` (~0). ⚠️ ≠ `f_ricavi_fb` (= **Produzione Netta MENSILE**, source `POWERBI_RICAVIFB`). ⚠️ **Ricavo camere canonico = classe `01ROOM` qui**, NON `f_pms_statistiche.revenue_room` (base terza: ratio 1,09/1,04 vs 01ROOM — scoperto nel gate di `v_booking_curve`, 2026-07-12); da `f_pms_statistiche` si prendono notti (`camere_vendute`) e capacità (MODALE sui giorni con vendite, mai MAX — i giorni di pre-apertura hanno inventario di config diverso).
...
- `f_prenotazioni_otb` + `f_consprev_mensile` — **le fotografie si ACCUMULANO** (`snapshot_date` in natural key, pace = confronto tra foto della stessa source); in `f_prenotazioni_otb` le varianti `dim_tipologia` (ASSEGNATA/VENDUTA/NESSUNA) sono la stessa foto su dimensioni diverse → **mai sommarle**. Sono FATTI osservati (foto del PMS), non proiezioni. Vedi `vault/decisions/2026-07-11_Promotion_Prenotazioni_Snapshot_Pace` + concept `BOOKING_PACE_E_BASI`.
