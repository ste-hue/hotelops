...
> **2026-07-11 checkpoint:** **Season forecast layer** (loop `season_forecast`): famiglia prenotazioni promossa a canonical — `f_prenotazioni_otb` (portafoglio OTB: **le fotografie si accumulano**, `snapshot_date` da intake_at + `dim_tipologia` ASSEGNATA/VENDUTA/NESSUNA entrambe in natural key, varianti della stessa foto MAI sommabili), `f_bookings_tipologia` (venduto giornaliero per tipologia, replace), `f_consprev_mensile` (rollup mensile Cons/Prev+Cons/BDG/AP per classe; **BU dal footer per negazione**, content-only). Parser in `ingest/flussi/ingest_{andamento_prenotazioni,bookings_tipologia,consprev_mensile}.py`; pre-create in `core/bq/load/create_prenotazioni_tables.py`. `DETTAGLIOPRENOTAZIONI`/`CONSPREVPAX` restano RAW_ONLY (no consumer). Decisione `vault/decisions/2026-07-11_Promotion_Prenotazioni_Snapshot_Pace` + concept `BOOKING_PACE_E_BASI` (pace solo foto-vs-foto stessa source; parità-camera = 2025+13,2%). **Rituale**: export "Andamento Prenotazioni" settimanale → intake → promote. Inoltre: cleanup tabelle morte pre-lineage (drop `f_affidamenti`/`f_mastrino_consolidato`/`v_ultima_data`), skill triage (KILL `hotelops-ops`), allowlist permessi in `.claude/settings.json`.
>
